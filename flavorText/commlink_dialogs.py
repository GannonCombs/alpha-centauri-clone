...
# Import your faction files (assuming they are in the same directory or package)
from flavorText.factionText.gaian import gaians_data
from flavorText.factionText.believe import believers_data
from flavorText.factionText.hive import hive_data
from flavorText.factionText.morgan import morgan_data
from flavorText.factionText.peace import peace_data
from flavorText.factionText.spartans import spartan_data
from flavorText.factionText.univ import university_data

# Map faction IDs to their data dictionaries
FACTION_FLAVOR_MAP = {
    'GAIANS': gaians_data,
    'BELIEVERS': believers_data,
    'HIVE': hive_data,
    'MORGAN': morgan_data,
    'PEACEKEEPERS': peace_data,
    'SPARTANS': spartan_data,
    'UNIVERSITY': university_data
}

# Dialog database - contains ONLY dialog from script.txt
# Format exactly as found in script.txt (lines 2422-5894)
DIALOGS = {
# Generic fallback
    'GENERIC': {
        'text': '"I see."',
...
class DialogSubstitution:
    ...
    def set_context(self, player_faction, ai_faction):
        ...
        print(f"pf: {player_faction}")
        print(f"pf name: {player_faction['name']}")

        # 1. Get the ID
        faction_id = ai_faction.get('id', player_faction['name'])

        # 2. Get the flavor data
        flavor = FACTION_FLAVOR_MAP.get(faction_id, {})

        # FIX: If for some reason 'flavor' is a tuple, grab the first element
        if isinstance(flavor, tuple):
            flavor = flavor[0]

        self.current_ai_flavor = flavor
    ...
    def get_dialog(self, dialog_id, player_faction, ai_faction):
        ...
        if raw_id in self.current_ai_flavor:
            ...
            # Fallback to the big script.txt database
            dialog_data = DIALOGS.get(dialog_id, DIALOGS.get('GENERIC', {'text': '...', 'responses': []}))
